...
namespace Commons
{
    public class OSystems
    {
        ...
        public void MLoadSearchLookUpEdit(DevExpress.XtraEditors.SearchLookUpEdit cbo, DataTable dtTmp, string Ma, string Ten, string form)
        {
            try
            {
                cbo.Properties.DataSource = null;
        ...
        public DataTable DataCaActive(bool coAll)
        {
            string sSql = "";
            if (coAll == true)
            {
                sSql = "SELECT STT,CA FROM dbo.CA WHERE ACTIVE = 1 UNION SELECT -1,'< ALL >' ORDER BY CA";
            }
            else
            {
                sSql = "SELECT STT,CA FROM dbo.CA WHERE ACTIVE = 1 UNION SELECT NULL,'' ORDER BY CA";
            }
