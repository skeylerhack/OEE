...
namespace OEE
{
    public partial class ucProductRun : DevExpress.XtraEditors.XtraForm
    {
        Int64 iThem = 0;
        ...
        private void ucProductRun_Load(object sender, EventArgs e)
        {
        ...
        private void Loadngay()
        {
            ...
            datDenNgay.DateTime = DateTime.Now.Date.AddDays(-DateTime.Now.Date.Day + 1).AddMonths(1).AddDays(-1);
        }
        ...
        private void LoadgrdEquiment()
        {
            ...
            try
            {
                dtmp.Load(SqlHelper.ExecuteReader(Commons.IConnections.CNStr, "spGetProDuctionOEE", rdoXemtheo.SelectedIndex == 0 ?  Convert.ToInt64(grvProDuctRun.GetFocusedRowCellValue("ID")) : -1 , cboCa.EditValue, datBD.DateTime, Commons.Modules.UserName, Commons.Modules.TypeLanguage));
        ...
        {
            iThem = -1;
            Commons.Modules.ObjSystems.MLoadLookUpEdit(cboCa, Commons.Modules.ObjSystems.DataCaActive(false), "STT", "CA", Commons.Modules.ObjLanguages.GetLanguage(this.Name, "Ca"), false);
            InsertResetText();
            txtCode.Focus();
        ...
        private void grvPrRunDetails_ValidatingEditor(object sender, BaseContainerValidateEditorEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //kiem tra co thoi gian ngung may chua chua thi khong cho sua
                    int n = Convert.ToInt32(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, CommandType.Text, "SELECT COUNT(*) FROM dbo.THOI_GIAN_NGUNG_MAY WHERE ProductionRunDetailsID = "+ grvPrRunDetails.GetFocusedRowCellValue("DetailID") + ""));
                    if(n >0)
                    {
                        //kiểm tra có nằm từ ngày nhỏ hơn min thời gian ngừng máy
                        DateTime min = Convert.ToDateTime(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, CommandType.Text, "SELECT MIN(NGAY + TU_GIO) FROM dbo.THOI_GIAN_NGUNG_MAY WHERE ProductionRunDetailsID = " + grvPrRunDetails.GetFocusedRowCellValue("DetailID") + ""));
                        if (DateTime.Compare(Convert.ToDateTime(Convert.ToDateTime(e.Value)), min) > 0)
                        {
                            e.Valid = false;
                            e.ErrorText = "delete dowtime";
                    ...
                    {
                        e.Valid = false;
                        e.ErrorText = "from date not to date";
                        view.SetColumnError(view.Columns["StartTime"], e.ErrorText);
                        return;
                    }
                    else
                ...
                if (view.FocusedColumn.Name == "colEndTime")
                {
                    ...
                    {
                        //kiểm tra có nằm từ ngày nhỏ hơn min thời gian ngừng máy
                        DateTime max = Convert.ToDateTime(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, CommandType.Text, "SELECT MAX(DEN_NGAY + DEN_GIO) FROM dbo.THOI_GIAN_NGUNG_MAY WHERE ProductionRunDetailsID = " + grvPrRunDetails.GetFocusedRowCellValue("DetailID") + ""));
                        if (DateTime.Compare(Convert.ToDateTime(e.Value), max) < 0)
                        {
                            e.Valid = false;
                            e.ErrorText = "delete dowtime";
        ...
        private void mnuNhapTGNM_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                frmNM.msProRun = Convert.ToInt64(grvProDuctRun.GetFocusedRowCellValue("ID"));
        ...
        private void rdoXemtheo_SelectedIndexChanged(object sender, EventArgs e)
        {
            LoadgrdEquiment();
        }
