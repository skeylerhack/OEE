...
namespace OEE
{
    public partial class frmThoiGianNgungMay : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void LoadgrdNgungMay(string ms, DateTime? tn)
        {
            ...
            try
            {
                dtmp.Load(SqlHelper.ExecuteReader(Commons.IConnections.CNStr, "spNguyenNhanNgungMayByProDetails", chkTheoproduction.Checked == true  ? msProRun  : -1, cboMsMay.EditValue, datTuNgay.DateTime,datDenNgay.DateTime,Commons.Modules.TypeLanguage));
        ...
        private bool SaveData()
        {
            ...
            if (datBD.DateTime >= datTNgay && datKT.DateTime <= datDNgay)
            {
            }
            else
            {
        ...
        private void chkTheoproduction_CheckedChanged(object sender, EventArgs e)
        {
            LoadgrdNgungMay("-1", null);
        }
    }
